...
import { useProTagFetch } from '@/composables'

const emit = defineEmits(['close', 'submit'])
const show = ref(false)
const loading = ref(false)
const formRef = ref()
const { proTagDefaultData, proTagAdd, proTagEdit } = useProTagFetch()
const formData = ref<any>({ ...proTagDefaultData })
const rules = {
  name: [
...
function doClose() {
  emit('close')
  show.value = false
  formData.value = { ...proTagDefaultData }
}
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await proTagEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await proTagAdd(formData.value)
        if (success) {
