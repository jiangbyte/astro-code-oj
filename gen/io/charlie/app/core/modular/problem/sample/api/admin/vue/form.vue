...
import { useProSampleLibraryFetch } from '@/composables'

const emit = defineEmits(['close', 'submit'])
const show = ref(false)
const loading = ref(false)
const formRef = ref()
const { proSampleLibraryDefaultData, proSampleLibraryAdd, proSampleLibraryEdit } = useProSampleLibraryFetch()
const formData = ref<any>({ ...proSampleLibraryDefaultData })
const rules = {
  userId: [
...
function doClose() {
  emit('close')
  show.value = false
  formData.value = { ...proSampleLibraryDefaultData }
}
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await proSampleLibraryEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await proSampleLibraryAdd(formData.value)
        if (success) {
