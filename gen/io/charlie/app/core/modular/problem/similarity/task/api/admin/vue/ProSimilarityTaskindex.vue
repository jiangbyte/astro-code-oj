...
const { proSimilarityTaskPage, proSimilarityTaskDelete } = useProSimilarityTaskFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await proSimilarityTaskPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await proSimilarityTaskDelete(param)
  if (success) {
