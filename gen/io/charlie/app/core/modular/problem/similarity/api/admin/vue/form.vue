...
const formRef = ref()
const { proSimilarityDetailDefaultData, proSimilarityDetailAdd, proSimilarityDetailEdit } = useProSimilarityDetailFetch()
const formData = ref<any>({ ...proSimilarityDetailDefaultData })
const rules = {
  taskId: [
...
function doClose() {
  emit('close')
  show.value = false
  formData.value = { ...proSimilarityDetailDefaultData }
}
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await proSimilarityDetailEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await proSimilarityDetailAdd(formData.value)
        if (success) {
