...
const formRef = ref()
const { proSimilarityTaskDefaultData, proSimilarityTaskAdd, proSimilarityTaskEdit } = useProSimilarityTaskFetch()
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await proSimilarityTaskEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await proSimilarityTaskAdd(formData.value)
        if (success) {
