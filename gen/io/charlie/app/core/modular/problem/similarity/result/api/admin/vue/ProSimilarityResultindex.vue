...
const { proSimilarityResultPage, proSimilarityResultDelete } = useProSimilarityResultFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await proSimilarityResultPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await proSimilarityResultDelete(param)
  if (success) {
