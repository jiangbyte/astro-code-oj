...
const formRef = ref()
const { proSetSimilarityResultDefaultData, proSetSimilarityResultAdd, proSetSimilarityResultEdit } = useProSetSimilarityResultFetch()
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await proSetSimilarityResultEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await proSetSimilarityResultAdd(formData.value)
        if (success) {
