...
const { proSetSimilarityResultPage, proSetSimilarityResultDelete } = useProSetSimilarityResultFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await proSetSimilarityResultPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await proSetSimilarityResultDelete(param)
  if (success) {
