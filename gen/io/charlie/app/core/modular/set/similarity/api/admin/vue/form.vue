...
import { useProSetSimilarityDetailFetch } from '@/composables'

const emit = defineEmits(['close', 'submit'])
const show = ref(false)
const loading = ref(false)
const formRef = ref()
const { proSetSimilarityDetailDefaultData, proSetSimilarityDetailAdd, proSetSimilarityDetailEdit } = useProSetSimilarityDetailFetch()
const formData = ref<any>({ ...proSetSimilarityDetailDefaultData })
const rules = {
  taskId: [
...
function doClose() {
  emit('close')
  show.value = false
  formData.value = { ...proSetSimilarityDetailDefaultData }
}
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await proSetSimilarityDetailEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await proSetSimilarityDetailAdd(formData.value)
        if (success) {
      ...
      <NForm ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="auto">
            ...
            <!-- 输入框 -->
              ...
              <NFormItem label="样本提交时间" path="originTime">
                <NDatePicker type="datetime" v-model:value="formData.originTime" />
              </NFormItem>
            <!-- 输入框 -->
