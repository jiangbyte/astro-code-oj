...
const { proSetSimilarityTaskPage, proSetSimilarityTaskDelete } = useProSetSimilarityTaskFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await proSetSimilarityTaskPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await proSetSimilarityTaskDelete(param)
  if (success) {
