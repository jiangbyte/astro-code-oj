...
const formRef = ref()
const { proSetSubmitDefaultData, proSetSubmitAdd, proSetSubmitEdit } = useProSetSubmitFetch()
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await proSetSubmitEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await proSetSubmitAdd(formData.value)
        if (success) {
