...
const { proSetSolvedPage, proSetSolvedDelete } = useProSetSolvedFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await proSetSolvedPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await proSetSolvedDelete(param)
  if (success) {
