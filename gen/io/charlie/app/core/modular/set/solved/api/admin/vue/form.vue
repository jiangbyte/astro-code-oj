...
const formRef = ref()
const { proSetSolvedDefaultData, proSetSolvedAdd, proSetSolvedEdit } = useProSetSolvedFetch()
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await proSetSolvedEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await proSetSolvedAdd(formData.value)
        if (success) {
