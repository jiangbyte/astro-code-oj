...
import io.charlie.app.core.modular.category.param.ProCategoryAddParam;
import io.charlie.app.core.modular.category.param.ProCategoryEditParam;
import io.charlie.app.core.modular.category.param.ProCategoryIdParam;
import io.charlie.app.core.modular.category.param.ProCategoryPageParam;
...
public class ProCategoryServiceImpl extends ServiceImpl<ProCategoryMapper, ProCategory> implements ProCategoryService {
    ...
    public Page<ProCategory> page(ProCategoryPageParam proCategoryPageParam) {
        ...
        if (ObjectUtil.isNotEmpty(proCategoryPageParam.getKeyword())) {
            queryWrapper.lambda().like(ProCategory::getTitle, proCategoryPageParam.getKeyword());
        }
        if (ObjectUtil.isAllNotEmpty(proCategoryPageParam.getSortField(), proCategoryPageParam.getSortOrder()) && ISortOrderEnum.isValid(proCategoryPageParam.getSortOrder())) {
            ...
            queryWrapper.lambda().orderByAsc(ProCategory::getSort);
        }
