...
public class ProSetSubmitController {
    ...
    public Result<?> detail(@Valid ProSetSubmitIdParam proSetSubmitIdParam) {
        return Result.success(proSetSubmitService.detail(proSetSubmitIdParam));
    }

    @Operation(summary = "执行代码")
    //@SaCheckPermission("/pro/set/submit/exec")
    @PostMapping("/pro/set/submit/exec")
    public Result<?> executeCode(@RequestBody @Valid ProSetSubmitExecParam proSetSubmitExecParam) {
        return Result.success(proSetSubmitService.executeCode(proSetSubmitExecParam));
    }
}