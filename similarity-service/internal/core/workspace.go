...
//	}
//
//	var taskSimilarities []*model.TaskSimilarity
//
//	for _, sample1 := range sampleLibraries {
//		for _, sample2 := range sampleLibraries {
//			// 不自检测
...
//			var sample1CodeTokens []int
//			if err := json.Unmarshal([]byte(sample1.CodeToken), &sample1CodeTokens); err != nil {
//				logx.Infof("解析 CodeToken1 JSON 失败 (ID: %s): %v", sample1.ID, err)
//				continue
//			}
//
//			var sample2CodeTokens []int
//			if err := json.Unmarshal([]byte(sample2.CodeToken), &sample2CodeTokens); err != nil {
//				logx.Infof("解析 CodeToken2 JSON 失败 (ID: %s): %v", sample1.ID, err)
//				continue
//			}
//
//			matches := similarity.GreedyStringTiling(sample1CodeTokens, sample2CodeTokens, w.similarityMessage.MinMatchLength)
...
//			taskSimilarity := w.createTaskSimilarity(sample1, sample2, similarityScore)
...
//
//// 创建 TaskSimilarity 记录
//func (w *Workspace) createTaskSimilarity(sample1 model.DataLibrary, sample2 model.DataLibrary, similarityScore float64) *model.TaskSimilarity {
//	now := time.Now()
//
//	sample1Token, _ := json.Marshal(sample1.CodeToken)
//	if sample1Token == nil {
//		sample1Token, _ = json.Marshal([]int{})
//	}
//
//	sample2Token, _ := json.Marshal(sample2.CodeToken)
//	if sample2Token == nil {
//		sample2Token, _ = json.Marshal([]int{})
//	}
//
//	sample1Name, _ := json.Marshal(sample1.CodeTokenName)
//	if sample1Name == nil {
//		sample1Name, _ = json.Marshal([]string{})
//	}
//
//	sample2Name, _ := json.Marshal(sample2.CodeTokenName)
//	if sample2Name == nil {
//		sample2Name, _ = json.Marshal([]string{})
//	}
//
//	sample1Text, _ := json.Marshal(sample1.CodeTokenTexts)
//	if sample1Text == nil {
//		sample1Text, _ = json.Marshal([]string{})
//	}
//
//	sample2Text, _ := json.Marshal(sample2.CodeTokenTexts)
//	if sample2Text == nil {
//		sample2Text, _ = json.Marshal([]string{})
//	}
//
...
//		SubmitCodeToken:  string(sample1Token),
//		SubmitTokenName:  string(sample1Name),
//		SubmitTokenTexts: string(sample1Text),
//
...
//		OriginCodeToken:  string(sample2Token),
//		OriginTokenName:  string(sample2Name),
//		OriginTokenTexts: string(sample2Text),
//
//		CreateTime: &now,
//		CreateUser: "0",
//		UpdateTime: &now,
...
//
//	totalPairs := len(validSamples) * (len(validSamples) - 1)
//	taskSimilarities = make([]*model.TaskSimilarity, 0, totalPairs)
//
//	for i, sample1 := range validSamples {
//		for j, sample2 := range validSamples {
//			if i == j { // 跳过自比较
//				continue
//			}
//
...
//
//			go func(s1, s2 model.DataLibrary, idx1, idx2 int) {
//				defer wg.Done()
