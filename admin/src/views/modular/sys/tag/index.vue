...
import { useSysTagFetch } from '@/composables'
...
const { sysTagPage, sysTagDelete } = useSysTagFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await sysTagPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await sysTagDelete(param)
  if (success) {
