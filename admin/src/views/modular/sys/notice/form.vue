...
const formRef = ref()
const { sysNoticeDefaultData, sysNoticeAdd, sysNoticeEdit } = useSysNoticeFetch()
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await sysNoticeEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await sysNoticeAdd(formData.value)
        if (success) {
...
<template>
  <NDrawer v-model:show="show" :mask-closable="false" placement="right" :default-width="1200" resizable @after-leave="doClose">
