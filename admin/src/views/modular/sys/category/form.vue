...
import { useSysCategoryFetch } from '@/composables/v1'

const emit = defineEmits(['close', 'submit'])
const show = ref(false)
const loading = ref(false)
const formRef = ref()
const { sysCategoryDefaultData, sysCategoryAdd, sysCategoryEdit } = useSysCategoryFetch()
const formData = ref<any>({ ...sysCategoryDefaultData })
const rules = {
  name: [
...
function doClose() {
  emit('close')
  show.value = false
  formData.value = { ...sysCategoryDefaultData }
}
...
async function doSubmit() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      loading.value = true
      if (isEdit.value) {
        const { success } = await sysCategoryEdit(formData.value)
        if (success) {
          window.$message.success('修改成功')
        }
      }
      else {
        const { success } = await sysCategoryAdd(formData.value)
        if (success) {
