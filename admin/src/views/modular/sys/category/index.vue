...
const { sysCategoryPage, sysCategoryDelete } = useSysCategoryFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await sysCategoryPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await sysCategoryDelete(param)
  if (success) {
