...
const columns: DataTableColumns<any> = [
  ...
  },
  {
    ...
    render: (row) => {
      return h(NTime, { time: row.operationTime })
    },
  },
  {
    title: '操作分类',
...
const { sysLogPage, sysLogDelete } = useSysLogFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await sysLogPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await sysLogDelete(param)
  if (success) {
