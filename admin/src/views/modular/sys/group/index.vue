...
const columnSortFieldOptions = computed<any[]>(() => {
  return [
    { label: 'ID', value: 'id' },
    { label: '排序', value: 'sort' },
    { label: '父级用户组', value: 'parentId' },
    { label: '系统组', value: 'groupType' },
...
const { sysGroupPage, sysGroupDelete } = useSysGroupFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await sysGroupPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await sysGroupDelete(param)
  if (success) {
