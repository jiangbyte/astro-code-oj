...
const { proSetSubmitPage, proSetSubmitDelete } = useProSetSubmitFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await proSetSubmitPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await proSetSubmitDelete(param)
  if (success) {
