...
const { proSetProgressPage, proSetProgressDelete } = useProSetProgressFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await proSetProgressPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await proSetProgressDelete(param)
  if (success) {
