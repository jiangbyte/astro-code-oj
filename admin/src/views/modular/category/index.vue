...
const { proCategoryPage, proCategoryDelete } = useProCategoryFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await proCategoryPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await proCategoryDelete(param)
  if (success) {
