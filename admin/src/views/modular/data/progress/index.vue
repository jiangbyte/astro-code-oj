...
const { dataProgressPage, dataProgressDelete } = useDataProgressFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await dataProgressPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await dataProgressDelete(param)
  if (success) {
