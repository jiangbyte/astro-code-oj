...
const props = defineProps<{
  problemId: string // 接收父组件传递的值
}>()

console.log('Case 题目ID-->', props.problemId)
    ...
    render(row: any) {
      return h(NSpace, { align: 'center' }, () => [
        h(NButton, {
          type: 'primary',
          size: 'small',
          onClick: () => formRef.value.doOpen(row, true, props.problemId),
        }, () => '编辑'),
...
const { dataTestCasePage, dataTestCaseDelete } = useDataTestCaseFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await dataTestCasePage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await dataTestCaseDelete(param)
  if (success) {
    ...
    <NCard size="small">
      <NSpace vertical>
        ...
        <NSpace align="center" justify="space-between">
          <NSpace align="center">
            <NButton type="primary" @click="formRef.doOpen(null, false, props.problemId)">
              <template #icon>
                <IconParkOutlinePlus />
              </template>
              创建
