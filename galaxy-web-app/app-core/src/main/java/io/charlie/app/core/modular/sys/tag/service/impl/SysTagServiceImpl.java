package io.charlie.app.core.modular.sys.tag.service.impl;
...
import io.charlie.app.core.modular.sys.tag.service.SysTagService;
...
public class SysTagServiceImpl extends ServiceImpl<SysTagMapper, SysTag> implements SysTagService {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void delete(List<SysTagIdParam> sysTagIdParams) {
        if (ObjectUtil.isEmpty(sysTagIdParams)) {
            throw new BusinessException(ResultCode.PARAM_ERROR);
        }
        sysTagIdParams.forEach(proTagIdParam -> proProblemTagService.deleteByTagId(proTagIdParam.getId()));
        this.removeByIds(CollStreamUtil.toList(sysTagIdParams, SysTagIdParam::getId));
    }

    @Override
    public SysTag detail(SysTagIdParam sysTagIdParam) {
        SysTag proTag = this.getById(sysTagIdParam.getId());
