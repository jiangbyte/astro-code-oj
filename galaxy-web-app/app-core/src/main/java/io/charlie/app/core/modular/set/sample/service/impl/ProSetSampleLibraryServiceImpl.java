package io.charlie.app.core.modular.set.sample.service.impl;
...
public class ProSetSampleLibraryServiceImpl extends ServiceImpl<ProSetSampleLibraryMapper, ProSetSampleLibrary> implements ProSetSampleLibraryService {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void addLibrary(ProSetSubmit proSetSubmit) {
        log.info("添加题目提交样本库 提交ID {}", proSetSubmit.getId());
        // 先判断是否已经存在库中
        if (this.exists(new LambdaQueryWrapper<ProSetSampleLibrary>()
                .eq(ProSetSampleLibrary::getUserId, proSetSubmit.getUserId())
                .eq(ProSetSampleLibrary::getSetId, proSetSubmit.getSetId())
                .eq(ProSetSampleLibrary::getProblemId, proSetSubmit.getProblemId())
        )) {
            // 存在则更新提交ID、提交时间、编程语言、源代码、源代码长度
            this.update(new LambdaUpdateWrapper<ProSetSampleLibrary>()
                    .eq(ProSetSampleLibrary::getUserId, proSetSubmit.getUserId())
                    .eq(ProSetSampleLibrary::getSetId, proSetSubmit.getSetId())
                    .eq(ProSetSampleLibrary::getProblemId, proSetSubmit.getProblemId())
                    .set(ProSetSampleLibrary::getSubmitId, proSetSubmit.getId())
                    .set(ProSetSampleLibrary::getSubmitTime, proSetSubmit.getCreateTime())
                    .set(ProSetSampleLibrary::getLanguage, proSetSubmit.getLanguage())
                    .set(ProSetSampleLibrary::getCode, proSetSubmit.getCode())
                    .set(ProSetSampleLibrary::getCodeLength, proSetSubmit.getCodeLength())
            );
        } else {
            // 不存在则新增

            ProSetSampleLibrary sampleLibrary = new ProSetSampleLibrary();
            sampleLibrary.setId(null);
            sampleLibrary.setUserId(proSetSubmit.getUserId());
            sampleLibrary.setSetId(proSetSubmit.getSetId());
            sampleLibrary.setProblemId(proSetSubmit.getProblemId());
            sampleLibrary.setSubmitId(proSetSubmit.getId());
            sampleLibrary.setSubmitTime(proSetSubmit.getCreateTime());
            sampleLibrary.setLanguage(proSetSubmit.getLanguage());
            sampleLibrary.setCode(proSetSubmit.getCode());
            sampleLibrary.setCodeLength(proSetSubmit.getCodeLength());
